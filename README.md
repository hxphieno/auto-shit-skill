...
这个 skill 做的，就是帮你的 AI Agent 养成上厕所的习惯——支持 Claude Code 和 OpenClaw。
...
**拉屎.skill 只清理 agent 自身的环境（Claude Code 或 OpenClaw），不碰你的项目代码。**
...
```bash
git clone https://github.com/hxphieno/auto-shit-skill ~/.openclaw/workspace/skills/auto-shit-skill
```

安装后重启 OpenClaw 即生效。

> ✅ **OpenClaw 已支持！** v1.1.0 起正式支持 OpenClaw 平台。使用相同的触发词，自动识别运行平台。
...
| `冲状态` / `冲环境` | 合并清理状态目录 |
| `冲插件` / `冲 extensions` | 合并清理插件/扩展 |
| `冲旧图` | OpenClaw 媒体文件清理 |
| `退旧房` | OpenClaw 废弃工作区清理 |
| `拔废管` | OpenClaw 废弃扩展清理 |
...
```
~/.openclaw/
├── workspace/memory/   ← 过时 memory 条目
├── agents/*/sessions/  ← 旧会话 transcript
├── cron/runs/          ← cron 运行日志
├── logs/               ← Gateway 日志
├── extensions/         ← 废弃扩展目录
├── media/              ← 过期媒体文件
